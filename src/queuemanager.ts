...
import rooms from "shared/services/room.js";
...
class QueueManager {
    ...
    async onJoinQueue(msg) {
        ...
        let existingParty = this.queuedParties[captain];
        if (existingParty) {
            for (let queue of existingParty.queues) {
                if (!queues.find(q => q.game_slug == queue.game_slug && q.mode == queue.mode))
                    queues.push(queue);
            }
        }
        ...
        for (const player of players) shortids.push(player.shortid);

        //find all player ratings for each game being queued
        let { playerRatings, invalidPlayers } = await ratings.findGroupRatings(shortids, game_slugs);
...
// const fakeusersPath = path.resolve(__dirname, "./fakeusers_final.json");
// const fakeusers = JSON.parse(fs.readFileSync(fakeusersPath, "utf8"));
